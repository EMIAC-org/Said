  ...
  return (
    ...
    >
      ...
      <p className="text-[10px] text-muted-foreground leading-relaxed opacity-70">
        AutoNote will remember this style for future recordings.
      </p>
...
export function VocabularyToast({ kind, term, source, onUndo, onDismiss }: VocabToastProps) {
  ...
  const subtle = (() => {
    if (kind === "starred") return "AutoNote will keep this even if you stop using it.";
    if (kind === "removed") return "AutoNote won't recognise this any more.";
    if (kind === "queued")  return "Make this fix once more and AutoNote will remember it.";
    return source === "manual"
      ? "AutoNote will recognise this on your next recording."
      : "AutoNote remembered your correction.";
  })();
