//! LLM polish client (Groq, OpenAI-compatible Chat Completions).
//!
//! Endpoint default: `https://api.groq.com/openai/v1/chat/completions`, Bearer
//! auth. SSE shape: `data: {choices:[{delta:{content}}]}`. Streaming yields
//! tokens over an mpsc channel; `polish_once` is the non-streaming batch variant.
