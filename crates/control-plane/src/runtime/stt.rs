...
//! (prerecorded REST) for the fallback path. nova-3, 16 kHz linear16, multi for
//! Hinglish.
...
/// Parse a Deepgram streaming message into `(transcript, is_final)` when it
/// carries a non-empty transcript; otherwise `None`.
...
/// Batch (prerecorded) transcription for the fallback path.
