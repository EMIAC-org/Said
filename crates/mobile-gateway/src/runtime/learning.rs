...
/// What to do with an observed edit.
#[derive(Debug, PartialEq, Eq)]
pub enum LearnDecision {
    /// Learn `spoken` (lowercased) → `canonical`.
    Learn { spoken: String, canonical: String },
    /// `spoken` is unsafe to remap (contains a common word) — record a block.
    Block { spoken: String },
    /// Nothing actionable (formatting-only, too large, numeric, etc.).
    Ignore,
}

/// Extract the minimal changed `(from, to)` span between two strings by trimming
/// the common (case-insensitive) word prefix and suffix.
...
/// Decide what to learn from `original` → `edited` (the kept text).
pub fn analyze_edit(original: &str, edited: &str) -> LearnDecision {
    ...
    // Only small term-shaped corrections (1-3 words each).
    if from_t.split_whitespace().count() > 3 || to_t.split_whitespace().count() > 3 {
        return LearnDecision::Ignore;
    }
    let spoken = from_t.to_ascii_lowercase();
    // Char-count (not byte length) so the 2-char minimum is script-independent.
    if spoken.chars().count() < 2 || to_t.chars().count() < 2 {
        return LearnDecision::Ignore;
    }
    // Case/formatting-only change → ignore.
    if spoken == to_t.to_ascii_lowercase() {
        return LearnDecision::Ignore;
    }
    // Pure number/punctuation changes are formatting, not aliases.
    if !from_t.chars().any(|c| c.is_alphabetic()) || !to_t.chars().any(|c| c.is_alphabetic()) {
        return LearnDecision::Ignore;
    }
    // Never let a common word become an alias key (kaisa/mein/time/can/go …).
    // Applied token-wise so multi-word spans like "go to" or "can do" — which the
    // resolver would otherwise rewrite globally — are blocked too.
...
/// True if `word` (a single lowercased token) is a common word that must never
/// become an alias key. Mirrors the plan's explicit examples plus frequent
/// English/Hinglish function words.
...
mod tests {
    ...
    #[test]
    fn extracts_middle_span() {
        let c = extract_change("send the mac ops update", "send the Macobs update");
        assert_eq!(c, Some(("mac ops".to_string(), "Macobs".to_string())));
    }
    ...
    fn blocks_multiword_span_with_common_token() {
        // "go to" is composed of common words — must be blocked, not learned.
        assert_eq!(
    ...
    fn ignores_identical() {
        assert_eq!(analyze_edit("same text", "same text"), LearnDecision::Ignore);
    }
}
