//! Mobile polish prompt builder.
//!
//! A compact distillation of the desktop voice prompt: faithful cleaning,
//! strict output-language enforcement, light style shaping, and explicit
//! preservation of the user's personal vocabulary. It is deliberately a
//! *cleaner*, never a conversational responder — questions and commands inside
//! the transcript are cleaned, not answered.

/// Build the system prompt for a dictation polish.
...
/// Build the user message (transcript wrapped in a plain fence — injection-safe).
...
mod tests {
    ...
    #[test]
    fn hinglish_prompt_blocks_devanagari_and_preserves_mix() {
    ...
    #[test]
    fn user_message_wraps_transcript_without_xml() {
        let msg = build_user_message("aaj bahut kaam tha", "hinglish");
        assert!(msg.contains("=== BEGIN TRANSCRIPT ==="));
        assert!(msg.contains("aaj bahut kaam tha"));
