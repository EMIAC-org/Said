...
mod macos {
    ...
    unsafe extern "C" {
        ...
        fn CFStringGetCString(s: *const c_void, buf: *mut i8, buf_size: i64, encoding: u32)
        -> bool;
    ...
    unsafe fn method2(app: *const c_void, _el: *const c_void) -> Result<String, String> {
        ...
        let result =
            unsafe { ax_get_string(el2 as *const _, "AXValue") }.map_err(|e| e.to_string());
    ...
    unsafe fn method4(_app: *const c_void, el: *const c_void) -> Result<String, String> {
        ...
        let mut out: *mut c_void = std::ptr::null_mut();
        let err =
            unsafe { AXUIElementCopyParameterizedAttributeValue(el, attr, range_val, &mut out) };
    ...
    pub fn run() {
...

        {
            if !unsafe { AXIsProcessTrusted() } {
                ...
                println!(
                    "   System Settings → Privacy & Security → Accessibility → add this terminal"
                );
                return;
            }
...
fn main() {
    eprintln!("test_ax_methods is a macOS-only Accessibility diagnostic.");
}
