//! macOS implementation: Accessibility (AX) text reads + CGEvent paste/typing.
//!
//! Six-strategy focused-text read stack (see `diagnose_focused_field`):
//!   1. Direct `AXValue`
//!   2. `AXEnhancedUserInterface` / `AXManualAccessibility` unlock then re-read
//!   3. `AXSelectedText`
//!   4. `AXNumberOfCharacters` + `AXStringForRange`
//!   5. BFS subtree traversal (max depth 4, max 64 visits)
//!   6. Cmd+A + Cmd+C clipboard fallback
...
mod ffi {
    ...
    unsafe extern "C" {
        ...
        pub fn AXUIElementCopyAttributeNames(element: *const c_void, out: *mut *mut c_void) -> i32;
...
unsafe fn run_method_4(el: *const c_void) -> AxMethodResult {
    ...
    let mut out: *mut c_void = std::ptr::null_mut();
    let err =
        unsafe { ffi::AXUIElementCopyParameterizedAttributeValue(el, attr, range_val, &mut out) };
